...
extern uint32_t  fl_page_size_g, fl_chip_size_g;
...
void fl_chip_erase(void){
  ...
  uint32_t prot;
	...
	if(die_ver == 0xf0)
	{
		FLASH_F->CFG = 0xfeed0001;
		FLASH_F->TP = 0x1d;
		FLASH_F->TE = 0x3d;
		FLASH_F->PROT_1 = 0xfeed0000;
		FLASH_F->PROT_2 = 0xfeed0000;
		FLASH_F->CMD_1 = FL_POR;
		while(FLASH_F->STA_1 & 0x1)
		{
			;
		}
		FLASH_F->CMD_1 = FL_ERASE_CHIP;
		while(FLASH_F->STA_1 & 0x1)
		{
			;
		}
		FLASH_F->CMD_2 = FL_POR;
		while(FLASH_F->STA_2 & 0x1)
		{
			;
		}
		FLASH_F->CMD_2 = FL_ERASE_CHIP;
		while(FLASH_F->STA_2 & 0x1)
		{
			;
		}
...
void fl_page_erase(uint32_t pg){
  ...
  uint32_t prot;
	...
	{
		if(pg > (CHIP_SIZE_C / fl_page_size_g + 4))
      return;
	}
	else
	{
		if(pg > (CHIP_SIZE / fl_page_size_g + 4))
      return;
	}
  ...
  prot = fl_t->PROT;
  fl_t->PGA = pg;
  fl_t->PROT = 0xfeedffff & (~(1 << (pg / (PROT_SIZE / fl_page_size_g))));
...
void fl_chip_prog(uint8_t * data){
  ...
  uint32_t prot, i;
	...
	if(die_ver == 0xf0)
	{
		...
		FLASH_F->PROT_1 = 0xfeed0000;
		FLASH_F->PROT_2 = 0xfeed0000;
		FLASH_F->TP = 0x1d;  //pro 1.8ms
		FLASH_F->TE = 0x3d;  //erase 3.8ms
		FLASH_F->CMD_1 = FL_POR;
		while(FLASH_F->STA_1 & 0x1)
		{
			;
...
// 		}
		...
		FLASH_F->CMD_2 = FL_CLR_HVPL;
		while(FLASH_F->STA_2 & 0x1)
		{
			;
		}
		for(i = 0; i < 256; i++)
			*(uint8_t *)(fl_mem_base + 0x00040000 + i) = data[i];		
		FLASH_F->CMD_2 = FL_PROG_CHIP;
		while(FLASH_F->STA_2 & 0x1)
		{
			;
		}		
