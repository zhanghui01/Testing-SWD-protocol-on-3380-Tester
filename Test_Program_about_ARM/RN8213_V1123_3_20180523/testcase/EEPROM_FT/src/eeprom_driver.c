...
#define EE_POR_NO_TBC_C         (0xfeed0000 | 0x1)
#define EE_TBC_C                (0xfeed0000 | 0x2)
#define EE_POR_C                (0xfeed0000 | 0x3)
#define EE_CLR_HVPL_C           (0xfeed0000 | 0x4)
#define EE_WAKEUP_C             (0xfeed0000 | 0x5)
#define EE_STANDBY_C            (0xfeed0000 | 0xf)
#define EE_ERASE_CHIP_C         (0xfeed0000 | 0x9)
#define EE_ERASE_PAGE_C         (0xfeed0000 | 0xa)
#define EE_PROG_CHIP_C          (0xfeed0000 | 0xc)
#define EE_PROG_PAGE_C          (0xfeed0000 | 0xe)
...
void ee_page_erase(uint32_t pg){
  ...
  if(pg > (CHIP_SIZE / PAGE_SIZE + 4))
    return;
  
  ee_check(ee_t);
  prot = ee_t->PROT;
	ee_t->PGA = pg;
  ee_t->PROT = 0xfeedffff & (~(1 << (pg / (PROT_SIZE / PAGE_SIZE))));
	
	if(((*sram_chipid & 0xf0000000) == 0xc0000000) || ((*sram_chipid & 0xf0000000) == 0xe0000000))
	{
		for(i = 0; i < PAGE_SIZE_C/4; i++)
	  {
	  	*(uint32_t *)(EE_MEM_BASE + 4*i) = 0x0;
	  }
	}
	else
	{
...
void ee_sector_erase(uint32_t sect){
  ...
  if(sect > (CHIP_SIZE / SECTOR_SIZE + 4))
    return;
  ...
  if(((*sram_chipid & 0xf0000000) == 0xc0000000) || ((*sram_chipid & 0xf0000000) == 0xe0000000))
	{
		for(i = 0; i < PAGE_SIZE_C/4; i++)
	  {
	  	*(uint32_t *)(EE_MEM_BASE + 4*i) = ((uint32_t *)data)[i];
	  }
	}
	else
	{
