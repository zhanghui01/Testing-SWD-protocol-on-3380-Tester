...
unsigned long atf_img_run(ps_atf_img pImg) {
  ...
  if(pImg->load_remap == FLASH_RESIDENT)  //remap reg = 0
  {
  ...
  else if(pImg->load_remap == EEPROM_RESIDENT)  //remap reg = 1
  {
      if(pImg->load_addr < 0x08000000)
			ack = atf_eeprom_img_load(pImg);
	   else if(pImg->load_addr < 0x10000000)
...
unsigned long atf_flash_wr(unsigned long to_addr,unsigned char * pBuf,unsigned long len) {
  ...
  for(i = 0; (i * 128) < len; i++)
  {
      ...
      atf_mip_wait_timeout(fl_addr, 100);
	   //atf_flash_wait(fl_addr);
		  if( (len - i*128) < 128  )  // the last one ,the number of data is less than 128 byte.
			{
				ack = swd_mem_wr8(fl_addr, pBuf + i * 128, (len - i*128));
			}
			else
...
//         ack = swd_connect(1);	 //cgf 2015-5-23
			  ack = swd_connect(0);	 //EMU 20161214
#else
        ack = swd_connect(0);
#endif
		 con_loop++;
