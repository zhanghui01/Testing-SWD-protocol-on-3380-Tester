...
#ifdef   _SWD_DRV_GLOBALS
	#define  SWD_DRV_EXT
#else
	#define  SWD_DRV_EXT  extern 
#endif
...
#define SWD_PARALLEL  0 
#define SWD_SERIAL    1

SWD_DRV_EXT unsigned long swd_num;

/*  LPC1759 SWD IO configuration
 //并行
 *    SWDCLK:  P1.25
 *    SWDIO :  P1.26
*/
...
#define __set_swd_pclk_as_out()   (LPC_GPIO1 -> FIODIR |= (1 << 25))
#define __set_swd_pclk_as_in()    (LPC_GPIO1 -> FIODIR &= (~(1 << 25)))


#define __set_swd_pclk_to_high()  {\
								                   LPC_GPIO1 -> FIOSET = (1 << 25);\
								                 }

#define __set_swd_pclk_to_low()   {\
								                   LPC_GPIO1 -> FIOCLR = (1 << 25);\
								                 }

//Define for Parallel Program.
#define __set_swd_pio_mask()      //(LPC_GPIO1->FIOMASK = (~((1 << 25) | (1 << 26))))
#define __clear_swd_pio_mask()    //(LPC_GPIO1->FIOMASK = 0x0)
#define __set_swd_pio_as_out()    (LPC_GPIO1->FIODIR |= (1 << 26))
#define __set_swd_pio_as_in()     (LPC_GPIO1->FIODIR &= (~(1 << 26)))

#define __swd_p_idle()             {\
                   __set_swd_pio_mask() ;\
									 __set_swd_pio_as_out();\
                   __set_swd_pclk_as_out();\
                   __set_swd_pclk_to_low();\
                   __set_swd_pio_to_low();\
                 }
#define __swd_p_exit()            {\
                   __clear_swd_pio_mask();\
								  }									 
								 
#define __swd_p_init()             {\
                   LPC_PINCON->PINSEL3 &= (~((0x3 << 18) | (0x3 << 20)));\
                   __swd_p_idle();\
                 }

#define __set_swd_pio_to_high()   {\
                   LPC_GPIO1 -> FIOSET |= (1 << 26);\
                 }

#define __set_swd_pio_to_low()    {\
                   LPC_GPIO1 -> FIOCLR |= (1 << 26);\
                 }							 

#define __set_swd_pio(a)          {\
	                if(a==0) LPC_GPIO1 -> FIOCLR |= (1 << 26);\
									else LPC_GPIO1 -> FIOSET |= (1 << 26);\
								}
// #define __set_swd_pio(a)              {\
// 	                if(a==0) LPC_GPIO0 -> CLR = (1 << 11);\
// 									else LPC_GPIO0 -> SET = (1 << 11);\
//                  }								
							 
static  unsigned long __get_swd_pio(void) __attribute__((always_inline));
static  unsigned long __get_swd_pio(void) {
#ifndef CGF
  __set_swd_pio_as_in();
#endif
  return ((LPC_GPIO1->FIOPIN >> 26) & 1);
}
...
//Define for Serial Program.
#define __set_swd_sclk_as_out()   (LPC_GPIO2 -> FIODIR |= (1 << 2))
#define __set_swd_sclk_as_in()    (LPC_GPIO2 -> FIODIR &= (~(1 << 2)))


#define __set_swd_sclk_to_high()  {\
								                   LPC_GPIO2 -> FIOSET = (1 << 2);\
								                 }

#define __set_swd_sclk_to_low()   {\
								                   LPC_GPIO2 -> FIOCLR = (1 << 2);\
								                 }

#define __set_swd_sio_mask()      {\
                            LPC_GPIO0->FIOMASK = (~(1 << 0));\
                            LPC_GPIO2->FIOMASK = (~(1 << 2));\
                            LPC_GPIO1->FIOMASK = (~(1 << 28));;\
                          }
#define __clear_swd_sio_mask()      {\
                            LPC_GPIO0 -> FIOMASK = 0x0;\
                            LPC_GPIO1 -> FIOMASK = 0x0;\
                            LPC_GPIO2 -> FIOMASK = 0x0;\
                          }
                            
#define __set_swd_sio_as_out()    {\
                                    DIRINPUT_SET1;\
                                    LPC_GPIO0 -> FIODIR |= (1 << 0);\
                                  }
#define __set_swd_sio_as_in()     {\
                                    DIRINPUT_SET0;\
                                    LPC_GPIO0 -> FIODIR &= (~(1 << 0));\
                                  }
...
#define __set_swd_sio_to_high()   {\
                   LPC_GPIO0 -> FIOSET = (1 << 0);\
                 }

#define __set_swd_sio_to_low()    {\
                   LPC_GPIO0 -> FIOCLR = (1 << 0);\
                 }

#define __set_swd_sio(a)              {\
	                if(a==0) LPC_GPIO0 -> FIOCLR |= (1 << 0);\
									else LPC_GPIO0 -> FIOSET |= (1 << 0);\
                 }

// #define __set_swd_sio(a)              {\
// 	                if(a==0) LPC_GPIO2 -> CLR = (1 << 15);\
// 									else LPC_GPIO2 -> SET = (1 << 15);\
//                  }
static  unsigned long __get_swd_sio(void) __attribute__((always_inline));
static  unsigned long __get_swd_sio(void) {
#ifndef CGF
  __set_swd_sio_as_in();
#endif
  return (LPC_GPIO0->FIOPIN & 1);
}
...
extern void mem_wr_reg32(unsigned long addr,unsigned long data);
extern unsigned long mem_rd_reg32(unsigned long addr);
extern void mem_wr_reg8(unsigned long addr,uint8_t data);
extern uint8_t mem_rd_reg8(unsigned long addr);
extern void ra9103_initail(void);
