...
void uartInit(void)
{
  LPC_SC->PCONP |= (1 << 4);           /* module clock */
  LPC_SC->PCLKSEL0 &= (~(3 << 8));     /* apb clock(0:CCLK/4, 1: CCLK, 2: CCLK/2, 3: CCLK/8)*/
  LPC_PINCON->PINSEL4 |= ((0x2 << 0) | (0x2 << 0));    /* multi-function */
  
  LPC_UART1->LCR = (1 << 7) | (3 << 0);
  LPC_UART1->DLM = 0; /* Baud(9600) = PCLK / [16 * (256 * DLM + DLL) * (1 + FDR.DivAddVal/FDR.MulVal)]*/
  LPC_UART1->DLL = 13;//108;
  LPC_UART1->FDR = (1 << 0) | (15 << 4);//(1 << 0) | (2 << 4);
  
  LPC_UART1->LCR = (3 << 0);   /* 8bit data width, 1bit stop, no parity */
}
